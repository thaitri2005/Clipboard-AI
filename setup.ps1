...
if (-not $isAdmin) {
    Write-Host "⚠️  This script works best with administrator privileges" -ForegroundColor Yellow
...
Write-Host "Step 3: Setting up AI model (phi3:mini - lightweight & SQL-capable)..." -ForegroundColor Green
...
try {
    ...
    if ($testResult) {
        Write-Host "✅ Ollama is responding correctly!" -ForegroundColor Green
    }
} catch {
    Write-Host "⚠️  Test query had issues, but setup may still work" -ForegroundColor Yellow
}
Write-Host ""
...
Write-Host "   - Model size: ~2.3GB, fast responses" -ForegroundColor White
Write-Host ""
